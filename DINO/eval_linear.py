...
def eval_linear(args):
    ...
    if args.evaluate:
        utils.load_pretrained_linear_weights(linear_classifier, args.pretrained_weights_linear, args.arch, args.patch_size)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            ...
            #wandb.log({"acc_val": test_stats["acc1"]})
            #wandb.log({"loss_val": test_stats["loss"]})
...
def train(model, linear_classifier, optimizer, loader, epoch, n, avgpool):
    ...
    for (inp, target) in metric_logger.log_every(loader, 20, header):
        ...
        #=============visulize training log in wandb===================
        wandb.log({"Loss":loss,
...
def validate_network(val_loader, model, linear_classifier, n, avgpool):
    ...
    for inp, target in metric_logger.log_every(val_loader, 20, header):
        ...
        #list_prob = np.concatenate((list_prob,prob),axis=0)
        list_target = np.concatenate((list_target,tar),axis=0)
        
        #list_inp = np.stack((list_inp,im),axis=0)
        (a,_,_,_)=inp.size()
        for i in range(0,a):
            list_im.append(inp[i])        
        ...
        #================visulize classification log in wandb=======================
        wandb.log({"Classifier_Loss":loss,
                   "Classfier_Accuracy":acc1})

    #wandb confuson matrix, roc curve, PR
    #wandb.sklearn.plot_roc(target, prob, ["Airgun", "BlackButton","CouplingHalf","Cross","Electricity12","Fork1","Fork2","Fork3","Gear1","Gear2","Hammer","Hook","Pin1","Pin2","Pinion","Plug"])
    #wandb.sklearn.plot_precision_recall(target, prob, ["Airgun", "BlackButton","CouplingHalf","Cross","Electricity12","Fork1","Fork2","Fork3","Gear1","Gear2","Hammer","Hook","Pin1","Pin2","Pinion","Plug"])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrained_weights_linear', default='', type=str, help="Path to pretrained weights of classifier to evaluate.")
