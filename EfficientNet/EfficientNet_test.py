...
def get_args_parser():
    parser = argparse.ArgumentParser('EfficientNet', add_help=False)
    parser.add_argument('--data_path', default='/path/to/imagenet/train/', type=str,
        help='Please specify path to the ImageNet training data.')
    parser.add_argument('--weights', default=".", type=str, help='Path to save logs and checkpoints.')
    return parser
parser = argparse.ArgumentParser('EfficientNet', parents=[get_args_parser()])
...
def test_model(model):
    ...
    #confusion matrix, update classes name if needed
...
model_ft.classifier[1] = nn.Linear(in_features=2560, out_features=15)
